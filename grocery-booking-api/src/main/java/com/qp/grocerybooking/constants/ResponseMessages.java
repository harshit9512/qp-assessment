package com.qp.grocerybooking.constants;

public class ResponseMessages {
	public static final String LOGIN_SUCCESS = "Login successful.";
	public static final String INCORRECT_PASSWORD = "Failed to login due to incorrect password.";
	public static final String USER_REGISTERED = "User registered.";
	public static final String ITEM_ADDED_SUCCESSFULLY = "Item added successfully.";
	public static final String DATA_FETCHED_SUCCESSFULLY = "Data fetched successfully.";
	public static final String ITEM_UPDATED_SUCCESSFULLY = "Item updated successfully.";
	public static final String INVENTORY_UPDATED_SUCCESSFULLY = "Inventory updated successfully.";
	public static final String ITEM_DOES_NOT_EXIST = "Item does not exist.";
	public static final String UNABLE_TO_DELETE_ITEM = "Unable to delete item.";
	public static final String ITEM_DELETED_SUCCESSFULLY = "Item deleted successfully.";
	public static final String INVALID_CREDENTIALS = "Invalid credentials.";
}
